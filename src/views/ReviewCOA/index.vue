<template>
  <div class="container-fluid">
      ...
      >
        ...
        <template #cell(action)="data">
          <b-button
            ...
            "
            class="edit-btn"
            @click="handleStatus(data && data.item && data.item.id, 'approve')"
          >
            <i class="ri-check-line" /> Approve
          </b-button>
          <b-button
            v-if="
              role &&
            ...
            "
            class="del-btn"
            @click="handleStatus(data && data.item && data.item.id, 'reject')"
          >
            <i class="ri-forbid-line" /> Reject
          </b-button>
          <b-button
            v-if="
              role &&
            ...
            "
            class="edit-btn"
            @click="handleStatus(data && data.item && data.item.id, 'reopen')"
          >
            <i class="ri-restart-line" /> Reopen
          </b-button>
          <b-button
            v-if="
              role &&
            ...
            "
            class="del-btn"
            @click="handleStatus(data && data.item && data.item.id, 'close')"
          >
            <i class="ri-close-line" /> Close
          </b-button>
        </template>
        <template #cell(coa_reviews)="data">
          <div>
            <b-badge
              class="border border-primary text-primary mr-1"
              variant="none"
              @click="getModalList(data && data.item && data.item.id)"
            >
              <i class="las la-eye" style="font-size: 14px; cursor: pointer">
                View COA
              </i>
            </b-badge>
            <i
              ...
              @click="getCommentsList(data && data.item && data.item.id)"
            />
          </div>
        </template>
        <template #empty>
    ...
    <b-modal
      ...
      :ok-disabled="true"
    >
      <div v-if="modal.type === 'coa_reviews'">
        <div class="clone" v-if="role === 'ADMIN'">
        ...
        <b-table
          ...
          striped
          hover
          :items="modal && modal.records && modal.records.coa_reviews"
          :busy="modal && modal.loading"
          :fields="modal && modal.columns && modal.columns.coa_reviews"
          show-empty
        >
      ...
      <div v-if="modal.type === 'comments'">
        <div>
          <form ref="form" @submit.stop.prevent="handleSubmit">
            <div v-for="item in modal.columns.comments" :key="item.id">
              <b-form-group
                v-if="item.type === 'text'"
                :label="item.label"
                :label-for="item.key"
              >
                <textarea
                  :name="item.key"
                  :id="item.key"
                  :value="modal.form[item.key]"
                  :placeholder="item.placeholder"
                  :rows="item.rows"
                  :max-rows="item.maxRows"
                  class="form-control"
                  style="height: 100%; line-height: 2"
                  @change="setCommentValue"
                ></textarea>
              </b-form-group>
            </div>
            <div>
              <b-button
                v-if="modal.form && !modal.form.comment_id"
                class="edit-btn"
                @click="handleComments('create')"
              >
                Add Comment
              </b-button>
              <b-button
        ...
        <div>
          <div
            v-for="item in modal.records.comments"
            :key="item.id"
            class="commentBox"
          >
            ...
            <div>
              <h4>
                {{
                  item.efinop_user &&
                  item.efinop_user.first_name &&
                  item.efinop_user.last_name &&
                  `${item.efinop_user.first_name} ${item.efinop_user.last_name}`
                }}
              </h4>
              ...
              <div
                v-if="
                  ...
                  item.efinop_user &&
                  item.efinop_user.id &&
                  sessionUser.id === item.efinop_user.id
                "
                style="display: flex; align-items: center"
              >
                <span
                  style="margin-right: 12px; color: #c1c1c1; cursor: pointer"
                  @click="initializeModalForm(item)"
                  ><i class="ri-pencil-line" /> Edit</span
                >
                <span
                  style="color: #c1c1c1; cursor: pointer"
                  @click="
                    handleComments('delete', { comment_id: item && item.id })
                  "
                  ><i class="ri-delete-bin-7-line" /> Delete</span
                >
              </div>
            </div>
          </div>
...
import { ReviewCOA } from "../../schema";
import api from "../../Utils/api";

const { columns, coa_reviews_columns, comments_columns } = ReviewCOA;
...
export default {
  ...
  data: () => ({
    ...
    modal: {
      ...
      type: false,
      list_api: {
        coa_reviews: "/coa-review/find-many-review",
        comments: "review-comment/find-many",
      },
      comments_api: {
        create: "/review-comment/comment",
        update: "review-comment/update",
        delete: "/review-comment/delete",
      },
      records: {
        coa_reviews: [],
        comments: [],
      ...
      form: {},
      columns: {
        coa_reviews: (coa_reviews_columns || []).filter((_) => _.viewRecord),
        comments: (comments_columns || []).filter((_) => _.editRecord),
      },
      currentPage: 1,
      rows: 0,
      perPage: 10,
  ...
  methods: {
    async getList() {
      ...
      const reqData = {
        ...
        where: {
          status: ["OPEN", "CLOSED", "REJECTED", "APPROVED"],
        },
      };
      api
        .post(`/review-request/find-many`, reqData)
        .then((res) => {
          if (
            res &&
            res.data &&
            res.data.data &&
            res.data.data.review_requests &&
            res.data.data.pagination
          ) {
            this.records =
              (res.data.data.review_requests &&
                res.data.data.review_requests.length > 0 &&
                res.data.data.review_requests.map((d) =>
                  Object.assign({}, { ...d.efinop_user }, d)
                )) ||
              [];
    ...
    async getModalList(id) {
      ...
      this.modal.type = "coa_reviews";

      if (id) this.modal.coa_review_id = id;
      ...
      const reqData = {
        ...
        where: {
          review_request_id: this.modal.coa_review_id,
        },
      };

      api
        .post(this.modal.list_api.coa_reviews, reqData)
        .then((res) => {
          if (
            res &&
            res.data &&
            ...
            this.modal.records.coa_reviews = res.data.data.coa_reviews || [];
            const { page, count } = res.data.data.pagination;
            this.modal.currentPage = page;
            this.modal.rows = count;
          } else {
            this.makeToast("danger", "Error", "Unable to fetch api records");
          }
        })
    ...
    async getCommentsList(id) {
      ...
      api
        .post(this.modal.list_api.comments, reqData)
        .then((res) => {
          if (
            res &&
            res.data &&
            ...
            this.modal.records.comments =
              (commentsRecords &&
                commentsRecords.length > 0 &&
                commentsRecords.map((c) =>
                  Object.assign({}, c, {
                    created_at: moment(c.created_at).format("LL h:m:s"),
                  })
                )) ||
              [];
            const { page, count } = res.data.data.pagination;
    ...
    async handleStatus(id, status) {
      await api
        ...
        .then(() => {
          this.getList();
          this.makeToast("success", "Success", "Request Updated Successfully");
        })
        .catch(() => {
          this.makeToast("danger", "Error", "Unable to update the request");
        });
    },
    async handleComments(type, formData) {
      const api_url = this.modal.comments_api[type] || false;
      const reqData =
        type === "create"
          ? {
              data: Object.assign({}, this.modal.form, {
                review_request_id: this.modal.coa_review_id,
              }),
            }
          : type === "update"
      ...
      if (api_url) {
        api
          ...
          .catch((Err) => {
            this.makeToast("danger", "Error", "Unable to Update the Comment");
          });
      }
    },
    async handleClone() {
      if (this.modal && this.modal.coa_review_id) {
        api
          .post(`/coa-review/clone-review-request`, {
            data: {
              review_request_id: this.modal.coa_review_id,
            },
          })
          .then((res) => {
            ...
            this.$router.replace(`/coa/upload`)
          })
          .catch((Err) => {
    ...
    initializeModalForm(data) {
      this.modal.form = Object.assign(
        {},
        { comment_id: data.id, comment: data.comment }
      );
    },
    ...
    handleModalPagination(currentPage) {
      this.modal.currentPage = currentPage;
      this.getModalList();
    },
    setCommentValue(e) {
      this.modal.form = Object.assign({}, this.modal.form, {
